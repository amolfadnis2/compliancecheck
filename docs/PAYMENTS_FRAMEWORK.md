...
- **O-5 (POSH client-side PDF, go-live prerequisite): CLOSED.** Added `GET /api/report/posh/[id]/pdf` (`src/app/api/report/posh/[id]/pdf/route.ts` + `src/lib/pdf/server-posh-report.ts`), mirroring `/api/report/[id]/pdf` but reading `posh_assessments` directly. Gates on email ownership match, then on `getEntitlement()` when `isPaymentLive('posh')` is true (no-op today while `live: false`). `handleDownloadReport` in `src/app/assessment/posh/page.tsx` now calls this route instead of generating client-side once payment is live and a persisted (non-`local_`) assessment id exists; the client-side path remains as the fallback for `live: false` and unconfigured-DB local assessments. The row→report mapping was extracted to `adaptPOSHRow()` in `report-data-adapter.ts` and the three POSH label maps moved to `src/lib/assessments/posh/posh-labels.ts` so client and server render identical "Prepared For" details. Covered by `tests/unit/posh-row-adapter.test.ts` and `tests/unit/server-posh-report.test.ts`. POSH can now be flipped `live: true` without reopening this gap.
